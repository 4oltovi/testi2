...
@section('content')
                        ...
                        <tbody>
                            @forelse($group->activeStudents->sortBy('user.last_name') as $index => $student)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>
                                        <a href="{{ route('management.students.show', $student) }}">
    ...
    <div class="mt-3">
        <a href="{{ route('management.groups.index') }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left me-1"></i> Бозгашт ба гурӯҳҳо
        </a>
    </div>
@endsection
